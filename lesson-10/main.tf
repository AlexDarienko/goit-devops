...
provider "helm" {
  kubernetes {
    ...
    exec { api_version = "client.authentication.k8s.io/v1beta1"; args = ["eks", "get-token", "--cluster-name", module.eks.cluster_name]; command = "aws" }
  }
}
...
module "s3_backend" { source = "./modules/s3-backend"; bucket_name = var.bucket_name; table_name = var.dynamo_table_name }
...
module "jenkins" { source = "./modules/jenkins"; cluster_name = module.eks.cluster_name; oidc_provider_arn = module.eks.oidc_provider_arn; oidc_provider_url = module.eks.oidc_provider_url; depends_on = [module.eks] }
